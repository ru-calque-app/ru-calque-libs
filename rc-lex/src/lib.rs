...
impl Found {
    ...
    /// каждом тексте и о сложности не говорят ничего. По этому признаку их отсекают и при
    /// разметке текста — иначе в личный словарь ученика попали бы `the` и `in`, а
    /// «обязательной лексикой» каждой колоды оказались бы артикли.
